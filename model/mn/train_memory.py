...
def train(config):
    ...
    opt_gen = optim.Adam(gen.parameters(), lr=config.lr, betas=(config.beta1, 0.999),
                         weight_decay=0.00001)  # todo 注意这里用到了config里的配置
    ...
    for epoch in range(1, config.epoch + 1):
        ...
        for iteration, batch in enumerate(training_data_loader, 1):
            ...
            netG = GCN.GNNLoss(opt=config)
            loss_GNN = sum(netG(restored[j], real_b, 256) for j in range(len(restored)))  # 第一个传入的是生成图，第二个是真实图

            loss = (loss_char) + (0.05 * loss_edge) + loss_GNN
            ...
            if iteration % config.print_every == 0:
                print(
                    "===> Epoch[{}]({}/{):  loss_char: {:.4f} loss_edge: {:.4f}".format(
                        epoch, iteration, len(training_data_loader), loss_char.item(), loss_edge.item()
                    ))
                log = {}
                log['epoch'] = epoch
                log['iteration'] = len(training_data_loader) * (epoch - 1) + iteration
                log['loss_char/loss'] = loss_char.item()
                log['loss_edge/loss'] = loss_edge.item()
                logreport(log)

        print('epoch', epoch, 'finished, use time', time.time() - epoch_start_time)
        with torch.no_grad():
            log_validation = test(config, validation_data_loader, gen, criterionMSE, epoch)  # 这里得到的是epoch,ssim和 psnr
